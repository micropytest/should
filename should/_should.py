...
class AssertThrowContext:
  ...
  Attributes:
    cls: Exception class expected to be raised.
    match: Message match to assert.
    exception: Exception raised and caught.
  """

  def __init__(self, cls: type[Exception], match: str | None = None):
    self.cls = cls
    self.match = match
    self.exception = None
  ...
  def __exit__(self, cls, value, tb):
    ...
    if cls is None:
      assert False, f"Expected {self.cls} to be raised."

    # (2) assert
    self.exception = value

    assert issubclass(cls, self.cls), (
      f"Expected error ({type(value)}) to be instance of {self.cls}."
    )

    if isinstance(match := self.match, str):
      assert re.search(match, str(value)) is not None, (
        f"Expected error message '{value}' to be like '{match}'."
      )
...
class AssertValue:
  ...
  def throw(
    ...
    Args:
      exc: Exception class to be raised and caught.
      match: Pattern to comply the message.
    """

    return AssertThrowContext(exc, match)
